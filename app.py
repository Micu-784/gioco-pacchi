...
def display_blue_prizes(container):
    ...
    with container:
        ...
        for prize in blue_prizes:
            ...
            # Blue prizes: use secondary style
...
def apply_custom_css():
    """Reduce general font sizes and metric sizes"""
    st.markdown(
        """
        <style>
        /* Smaller title and headers */
        h1 {
        ...
        /* Metrics: shrink labels and values, reduce padding */
        ...
        .stMarkdown, .stText, .stDataFrame, .stRadio, .stNumberInput label {
...
def main():
    ...
    # Top controls: scenario selection + reset
    ...
    # Center column: offer + goal + indicators + history
    with center_col:
        st.markdown("#### 💰 Doctor's Offer & 🎯 Goal")
        
        # Row with Offer and Goal inputs
        ...
        # Row with Record Offer + Box Change + current offer/flag
        ...
        with btn_col1:
            if st.button("📝 Record Offer", use_container_width=True, type="primary"):
                if offer_input > 0:
                    ...
                    st.rerun()
        with btn_col2:
            if st.button("🔄 Box Change", use_container_width=True):
                mark_box_change()
                st.rerun()
        with btn_col3:
            if st.session_state.current_offer:
                st.caption(f"Current offer: €{st.session_state.current_offer:,.0f}")
            if st.session_state.box_changed_flag:
                st.caption("✅ Box changed")
